...
def check_keydown_events(event, ai_settings, screen, ship, bullets):
    ...
    if event.key == pygame.K_RIGHT:
        # Set the ship to move right
        ship.moving_right = True
    elif event.key == pygame.K_LEFT:
        # Set the ship to move left
...
def check_keyup_events(event, ship):
    if event.key == pygame.K_RIGHT:
        # Set the ship to stop moving right
        ship.moving_right = False
    elif event.key == pygame.K_LEFT:
        # Set the ship to stop moving left
...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, bullets, play_button):
    """Function to redraw the screen and ship during every game loop"""
...
def check_bullets_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    # check for any bullets which have hit aliens
...
def update_aliens(ai_settings, stats, sb, screen, ship, aliens, bullets):
    """Check if an alien is at an edge, then update position of all aliens in the fleet"""
...
def ship_hit(ai_settings, stats, sb, screen, ship, aliens, bullets):
    """Respond to alien-ship collisions; reduce no. of ships left, clear bullets, reset fleet and ship position"""
    # check if any ships repain
    if stats.ships_left > 0:

        # reduce number of ships remaining
        stats.ships_left -= 1
...
def check_aliens_bottom(ai_settings, stats, sb, screen, ship, aliens, bullets):
    """function to check if an alien hits the bottom of the screen and respond as if ship is hit"""
    # loop through aliens and check if any are at bottom of screen
    for alien in aliens.sprites():
        if alien.rect.bottom >= ai_settings.screen_height:
            # alien has hit bottom; respond as if ship hit
            ship_hit(ai_settings, stats, sb, screen, ship, aliens, bullets)
            break
